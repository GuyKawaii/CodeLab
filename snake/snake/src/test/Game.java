...
public class Game implements KeyListener {
  ...
  public static final int boardSpacePixels = 20;
  
  public static int updateSPeed = 100;
  public static int updateMinSPeed = 20;
  ...
  public Game() {
    ...
    window.setSize(boardWidth * boardSpacePixels + 2, boardHeight * boardSpacePixels + boardSpacePixels + 4);
  ...
  private boolean check_wall_collision() {
    if (player.getX() < 0 || player.getX() >= boardWidth * boardSpacePixels
        || player.getY() < 0 || player.getY() >= boardHeight * boardSpacePixels) {
      return true;
    }
    return false;
  }
  ...
  private boolean check_food_collision() {
    if (player.getX() == food.getX() * boardSpacePixels && player.getY() == food.getY() * boardSpacePixels) {
      updateSPeed = updateSPeed - 10;
      if (updateSPeed < updateMinSPeed) {
        updateSPeed = updateMinSPeed;
      }
      return true;
    }
    return false;
